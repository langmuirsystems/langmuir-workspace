#!/usr/bin/env bash
# push.sh — standard one-command commit+push for ANY repo registered in
# scripts/config.sh. Generalizes push-pms.sh, keeping the fixes for every
# snag we've hit:
#   • stale .git/index.lock from an interrupted git process (the cloud sandbox
...
# Show what's about to go, then commit only if there's something new.
